...
import { supabaseError, supabaseRest, supabaseAdminHeaders } from "../../lib/supabaseServer";
...
export async function GET(request: Request) {
  try {
    ...
    if (!isEffectiveRestEmail(session.email)) return NextResponse.json({ error: "No autorizado." }, { status: 403 });
    ...
    if (!headers) return NextResponse.json({ error: "Falta configurar el acceso del servidor a los archivos de descanso." }, { status: 503 });
