...
export type ZkiCrewRow = { plate: string; driver: string; driverId: string };

export async function readZkiCrewRows(headers: Record<string, string>) {
  const response = await fetch(supabaseRest(ZKI_CREW_TABLE, "?select=PLACA,CONDUCTOR,Cedula&limit=3000"), { headers, cache: "no-store" });
