...
export default function EffectiveRestAccessPage() {
  ...
  async function search(event: FormEvent) {
    ...
    prepareAudio();
    await lookup(document);
  }

  async function lookup(value: string) {
    if (inFlight.current) return;
    setResult(null); setAlertVisible(false);
    const id = value.trim();
    if (!/^\d{1,15}$/.test(id)) { setError("El código debe contener solo la cédula."); return; }
    setDocument(id);
    ...
    try {
      const response = await fetch(`/api/effective-rest?document=${encodeURIComponent(id)}`, { cache: "no-store" });
  ...
  function prepareAudio() {
    ...
    } catch { /* The visual result remains available without audio support. */ }
  }
