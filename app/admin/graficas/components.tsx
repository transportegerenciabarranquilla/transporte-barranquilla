...
export function ContractorRefusalHistory({ data }: { data: ContractorRefusalTrend[] }) {
  const colors = ["#f28b82", "#f6c667", "#b8a1ef", "#67c9d8", "#efa7c6"];
  ...
  return <section className="mb-4 overflow-hidden rounded-2xl border border-slate-200 bg-white p-5 text-slate-900 shadow-lg shadow-slate-200/60"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[.16em] text-rose-400">Histórico diario</p><h2 className="mt-1 text-lg font-black text-[#10223d]">Refusal diario por contratista</h2><p className="mt-1 text-xs text-slate-500">Porcentaje final diario: usa check-in por DT y, si no existe, el pendiente modulado</p></div><div className="flex flex-wrap gap-3">{data.map((row, index) => <span className="flex items-center gap-1.5 text-[10px] font-bold text-slate-600" key={row.contractor}><i className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: colors[index % colors.length] }} />{row.contractor}</span>)}</div></div>{dates.length ? <div className="mt-4 overflow-x-auto rounded-xl bg-slate-50/60"><svg aria-label="Barras de refusal diario por contratista" className="min-w-[680px]" role="img" style={{ minWidth: width }} viewBox={`0 0 ${width} ${height}`}>{[0, 1, 2, 3].map((value) => <g key={value}><line stroke="#dbe4ef" strokeDasharray="4 5" x1={left} x2={width-right} y1={y(value)} y2={y(value)} /><text fill="#64748b" fontSize="9" textAnchor="end" x={left-7} y={y(value)+3}>{value}%</text></g>)}{data.map((row, seriesIndex) => { const color = colors[seriesIndex % colors.length]; return <g key={row.contractor}>{row.points.map((point) => { const displayed = Math.min(point.percentage, maximum); const topY = y(displayed); const barHeight = Math.max(y(0) - topY, point.percentage > 0 ? 2 : 0); const centerX = barX(point.date, seriesIndex) + barWidth / 2; return <g key={point.date}><rect fill={color} height={barHeight} rx="4" width={Math.max(barWidth - 2, 5)} x={barX(point.date, seriesIndex) + 1} y={y(0) - barHeight}><title>{`${row.contractor}: ${point.percentage.toFixed(2)}% · ${point.pending} cajas final / ${point.dispatched} despachadas · ${formatDateLabel(point.date)}`}</title></rect>{point.percentage > 0 ? <text fill="#334155" fontSize="7.5" fontWeight="700" textAnchor="middle" x={centerX} y={Math.max(topY - 5, 10)}>{point.percentage.toFixed(2)}%</text> : null}</g>; })}</g>; })}{dates.map((date) => <text fill="#64748b" fontSize="9" textAnchor="middle" x={dateCenter(date)} y={height-12} key={date}>{date.slice(5)}</text>)}</svg></div> : <div className="mt-4 grid h-56 place-items-center rounded-xl border border-dashed border-slate-200 bg-slate-50 text-sm text-slate-500">Sin datos de refusal para el período seleccionado.</div>}</section>;
}
