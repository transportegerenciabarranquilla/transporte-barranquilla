...
export function RrRefusalTop({ data }: { data: RrRefusalSummary[] }) {
  ...
  return <>
    ...
    {selected ? <div aria-modal="true" className="fixed inset-0 z-50 grid place-items-center bg-slate-950/65 p-4 backdrop-blur-sm" onMouseDown={(event) => { if (event.target === event.currentTarget) setSelected(null); }} role="dialog"><article className="max-h-[85vh] w-full max-w-3xl overflow-hidden rounded-2xl bg-white shadow-2xl"><header className="flex items-start justify-between gap-4 border-b border-slate-200 bg-slate-50 p-5"><div><p className="text-[10px] font-black uppercase tracking-[.15em] text-red-600">Detalle del RR · {selected.contractor}</p><h3 className="mt-1 text-xl font-black text-[#10223d]">{selected.rr}</h3><p className="mt-1 text-xs text-slate-500">Clientes ordenados por número de veces que rechazaron</p></div><button aria-label="Cerrar detalle" className="grid h-9 w-9 place-items-center rounded-full border border-slate-200 bg-white text-slate-500 hover:bg-slate-100" onClick={() => setSelected(null)} type="button"><X size={17} /></button></header><div className="grid grid-cols-3 gap-2 border-b border-slate-100 p-4"><ModalTotal label="Clientes" value={selected.clientes.length} /><ModalTotal label="Rechazos" value={selected.registros} /><ModalTotal label="Cajas" value={selected.rechazadas} /></div><div className="max-h-[55vh] overflow-auto divide-y divide-slate-100">{selected.clientes.map((client, index) => <div className="grid grid-cols-[20px_minmax(0,1fr)_40px_60px] sm:grid-cols-[30px_minmax(0,1fr)_70px_90px] items-center gap-2 px-3 py-3 sm:gap-3 sm:px-5" key={client.codigo}><span className="grid h-7 w-7 place-items-center rounded-lg bg-slate-100 text-xs font-black text-slate-500">{index + 1}</span><div className="min-w-0"><p className="truncate text-xs font-black text-slate-800" title={client.nombre}>{client.nombre}</p><p className="mt-0.5 truncate text-[9px] font-semibold text-slate-400">{client.codigo} · {client.fechas.sort().map(formatDateLabel).join(", ")}</p></div><div className="text-right"><p className="text-lg font-black text-violet-700">{client.veces}x</p><p className="text-[8px] font-bold uppercase text-slate-400">veces</p></div><div className="text-right"><p className="text-lg font-black text-red-700">{client.rechazadas.toLocaleString("es-CO")}</p><p className="text-[8px] font-bold uppercase text-slate-400">cajas</p></div></div>)}</div></article></div> : null}
  </>;
}
...
export function ContractorRefusalHistory({ data }: { data: ContractorRefusalTrend[] }) {
  ...
  const highest = data.reduce((max, row) => row.points.reduce((value, point) => Math.max(value, point.percentage), max), 0);
  const tickStep = Math.max(1, Math.ceil(highest / 3));
  const maximum = tickStep * 3;
...
export function RefusalComBars({ data, emptyText }: { data: RefusalComSummary[]; emptyText: string }) {
  ...
  return (
    <div className="space-y-1.5">
      {data.map((item, index) => (
        <div className="grid grid-cols-[minmax(0,1fr)_minmax(40px,1fr)_auto] items-center gap-2 sm:grid-cols-[minmax(112px,170px)_1fr_52px]" key={`${item.contractor}-${item.label}`}>
          <div className="min-w-0">
  ...
  return (
    <div className="space-y-1.5">
      {data.map((item, index) => {
        ...
        return (
          <div className="grid grid-cols-1 items-center gap-2 rounded-md bg-slate-50 px-2 py-2 ring-1 ring-slate-100 sm:grid-cols-[minmax(108px,150px)_1fr_96px]" key={item.causal}>
            <div className="min-w-0">
