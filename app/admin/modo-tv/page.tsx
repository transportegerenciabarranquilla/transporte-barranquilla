...
export default function AdminModoTvPage() {
  ...
  return (
    <main className="min-h-screen bg-[#f4f7fb] text-[#10223d]">
        ...
        <div className="flex items-center justify-between gap-4 px-6 py-5 lg:px-10 2xl:px-14 2xl:py-7">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <h1 className="text-3xl font-bold lg:text-4xl 2xl:text-5xl">Seguimiento Logísticos</h1>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <span className="hidden text-sm text-slate-500 sm:inline 2xl:text-base">Hoy {formatDate(today)} · Actualizado {updated || "—"}</span>
        ...
        <div className="grid gap-7 xl:grid-cols-[1.25fr_1.75fr] 2xl:gap-9">
          <section className="rounded-xl border border-slate-200 bg-white p-7 shadow-sm 2xl:p-9">
            ...
            <div className="mt-8 grid grid-cols-3 gap-3 sm:gap-5 2xl:gap-8">
              {progressItems.map((item, index) => <ProgressBubble accent={index === 2 ? "#0f9f83" : index === 0 ? "#0f7c58" : "#2563eb"} key={item.label} label={item.label} value={item.value} />)}
            </div>
...
function ProgressBubble({ label, value, accent }: { label: string; value: number; accent: string }) { const safe = Math.min(100, Math.max(0, value)); return <div className="flex flex-col items-center gap-3"><div className="grid aspect-square w-full max-w-[210px] place-items-center rounded-full p-3 shadow-sm 2xl:max-w-[270px]" style={{ background: `conic-gradient(from -90deg, ${accent} ${safe}%, #e2e8f0 0)` }}><div className="grid h-full w-full place-items-center rounded-full bg-white text-center shadow-inner"><strong className="text-3xl font-bold tabular-nums 2xl:text-5xl">{safe.toFixed(1)}%</strong><span className="text-[9px] font-bold uppercase tracking-wider text-slate-400 2xl:text-xs">avance</span></div></div><span className="text-center text-sm font-bold 2xl:text-lg">{label}</span></div>; }
