...
export default function PreventaPage() {
  ...
  return <main className="min-h-screen bg-[#f3f5f8] text-slate-900">
    ...
    <section className="mx-auto max-w-[1500px] space-y-5 px-4 py-6 sm:px-8">
      ...
      <section className="flex h-[470px] min-w-0 flex-col overflow-hidden rounded-xl border border-violet-100 bg-white shadow-[0_12px_30px_-24px_rgba(76,29,149,.4)]">
        <div className="flex min-h-[64px] items-center justify-between border-b border-violet-100 bg-gradient-to-r from-violet-50/80 via-white to-white px-2.5 py-2"><div className="min-w-0"><p className="text-[7px] font-black uppercase tracking-[.14em] text-violet-600">Tabla 1 · Modulacion</p><h2 className="truncate text-sm font-black text-[#101c36]">Top 10 por número de rechazos</h2><p className="truncate text-[8px] text-slate-500">Histórico completo de Modulación.</p></div><span className="ml-2 grid h-7 w-7 shrink-0 place-items-center rounded-md bg-violet-100 text-[9px] font-black text-violet-700">10</span></div>
        {loading ? <div className="grid min-h-0 flex-1 place-items-center text-[10px] font-semibold text-slate-500">Consultando Modulacion...</div> : modulationTableRows.length === 0 ? <div className="grid min-h-0 flex-1 place-items-center p-6 text-center text-[10px] text-slate-500">No hay rechazos registrados en Modulacion.</div> : <div className="min-h-0 flex-1 overflow-auto"><table className="w-full min-w-[440px] table-fixed text-left text-[9px]"><thead className="sticky top-0 z-10 bg-slate-50 text-[7px] uppercase tracking-wide text-slate-500"><tr><th className="w-7 px-1.5 py-1.5">#</th><th className="w-[15%] px-1 py-1.5">Código</th><th className="w-[29%] px-1 py-1.5">Cliente</th><th className="w-[20%] px-1 py-1.5">Teléfono</th><th className="w-[16%] px-1 py-1.5">Contratista</th><th className="w-[9%] px-1 py-1.5 text-right">Veces</th><th className="w-[11%] px-1.5 py-1.5 text-right">Cajas</th></tr></thead><tbody className="divide-y divide-slate-100">{modulationTableRows.map((row, index) => <tr key={`${row.contractor}-${row.client_code}`} className="h-7 cursor-pointer transition hover:bg-violet-50/60" onClick={() => setSelectedOffender(row)}><td className="px-1.5 py-0.5"><span className={`grid h-4 w-4 place-items-center rounded text-[7px] font-black ${index < 3 ? "bg-violet-600 text-white" : "bg-slate-100 text-slate-500"}`}>{index + 1}</span></td><td className="truncate px-1 py-0.5 font-black text-[#101c36]">{row.client_code}</td><td className="truncate px-1 py-0.5 font-semibold text-slate-700" title={row.client_name}>{row.client_name || "Sin nombre"}</td><td className="truncate px-1 py-0.5 font-semibold text-slate-600">{row.phone || "Sin telefono"}</td><td className="truncate px-1 py-0.5 text-slate-500">{row.contractor}</td><td className="px-1 py-0.5 text-right"><span className="rounded-full bg-violet-100 px-1 py-0.5 font-black text-violet-700">{row.refusals}x</span></td><td className="px-1.5 py-0.5 text-right font-black text-rose-600">{Number(row.rejected_boxes || 0).toLocaleString("es-CO")}</td></tr>)}</tbody></table></div>}
      </section>
      <section className="flex h-[470px] min-w-0 flex-col overflow-hidden rounded-xl border border-blue-100 bg-white shadow-[0_12px_30px_-24px_rgba(37,99,235,.4)]">
      ...
      <section className="flex h-[470px] min-w-0 flex-col overflow-hidden rounded-xl border border-emerald-100 bg-white shadow-[0_12px_30px_-24px_rgba(5,150,105,.4)]">
