...
type RecordRow = { id: string; contractor: string; batch_id: string; batch_name: string; client_code: string; client_name: string; previous_refusals: number; products?: ProductLine[]; call_result: "pendiente" | "si" | "no"; no_contact_reason: string; notes: string; caller_name?: string; last_edited_by?: string; edit_history?: EditEntry[]; called_at?: string; created_at: string };
...
export default function PreventaPage() {
  ...
  const visible = useMemo(() => rows.filter((row) => {
    const text = `${row.client_code} ${row.client_name}`.toLowerCase();
  ...
  const callQueue = useMemo(
    () => [...visible]
      .sort((a, b) => b.previous_refusals - a.previous_refusals || a.client_name.localeCompare(b.client_name, "es"))
      .slice(0, 100),
    [visible],
  );
  const volumeClients = useMemo(() => historicalOffenders.slice().sort((a, b) => b.rejected_boxes - a.rejected_boxes || b.refusals - a.refusals).slice(0, 10), [historicalOffenders]);
  ...
  async function upload(selectedFiles: File[]) {
    ...
    if (selectedFiles.length > 3) { setError("Puedes subir máximo tres archivos de preventa a la vez."); return; }
    ...
    try {
      ...
      for (const file of selectedFiles) {
        ...
        if (!response.ok) throw new Error(`${file.name}: ${body.error || "No se pudo importar."}`);
        imported += Number(body.imported || 0);
        uploadedNames.push(body.fileName || file.name);
        lastBatchId = body.batchId || lastBatchId;
      }
      setMessage(`${imported} clientes cargados desde ${uploadedNames.length} archivo${uploadedNames.length === 1 ? "" : "s"}.`);
      await load(dateFrom, dateTo);
      setBatch(uploadedNames.length === 1 ? lastBatchId : "todos");
  ...
  async function refreshCalls() {
    setMessage("");
    await load(dateFrom, dateTo);
    setMessage("Top 100 actualizado con los clientes cargados en preventa.");
  }
    ...
    <section className="mx-auto max-w-[1500px] space-y-5 px-4 py-6 sm:px-8">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div><p className="text-xs font-bold uppercase tracking-[.18em] text-violet-600">Base de llamadas</p><h2 className="mt-1 text-2xl font-bold text-[#101c36]">Clientes programados para preventa</h2><p className="mt-1 text-sm text-slate-500">La plantilla solo requiere el código y el nombre del cliente; los rechazos se cruzan automáticamente.</p></div>
          <div className="flex flex-wrap gap-2">
            ...
            <input ref={fileRef} className="hidden" type="file" accept=".xlsx,.xls" multiple onChange={(e) => void upload(Array.from(e.target.files || []))}/>
            <button disabled={uploading} onClick={() => fileRef.current?.click()} className="inline-flex h-11 items-center gap-2 rounded-lg bg-violet-600 px-4 text-sm font-bold text-white hover:bg-violet-700 disabled:bg-slate-400"><Upload size={17}/>{uploading ? "Importando..." : "Subir hasta 3 preventas"}</button>
          </div>
        </div>
      </section>
      ...
      <section className="flex h-[620px] min-w-0 flex-col overflow-hidden rounded-xl border border-blue-100 bg-white shadow-[0_12px_30px_-24px_rgba(37,99,235,.4)] xl:col-span-3">
        <div className="flex min-h-[62px] items-center justify-between gap-2 border-b border-blue-100 bg-gradient-to-r from-blue-50/80 via-white to-white px-3 py-2"><div><p className="text-[8px] font-black uppercase tracking-[.16em] text-blue-600">Clientes cargados en preventa</p><h2 className="text-sm font-black text-[#101c36]">Top 100 clientes que más veces han rechazado</h2><p className="text-[9px] text-slate-500">Ordenados por cantidad histórica de rechazos registrados en Modulación.</p></div><div className="flex items-center gap-1.5"><span className="rounded-full bg-blue-100 px-2 py-1 text-[9px] font-black text-blue-700">{callQueue.length} clientes</span><button className="inline-flex h-7 items-center gap-1 rounded-md bg-blue-600 px-2 text-[9px] font-black text-white hover:bg-blue-700" disabled={loading || Boolean(saving)} onClick={() => void refreshCalls()} type="button"><RefreshCw size={11}/>Actualizar</button></div></div>
        ...
        {loading ? <div className="grid min-h-0 flex-1 place-items-center text-xs font-semibold text-slate-500">Cargando preventa...</div> : callQueue.length === 0 ? <div className="grid min-h-0 flex-1 place-items-center p-8 text-center"><div><span className="mx-auto grid h-14 w-14 place-items-center rounded-2xl bg-blue-50 text-blue-500"><Phone size={24} /></span><p className="mt-3 text-sm font-black text-slate-700">No hay clientes cargados con estos filtros</p></div></div> : <div className="min-h-0 flex-1 overflow-auto"><table className="w-full table-fixed text-left text-[10px]"><thead className="sticky top-0 z-10 bg-slate-50 text-[8px] uppercase tracking-wide text-slate-500"><tr><th className="w-[30%] px-2 py-1">Cliente</th><th className="w-[12%] px-1 py-1">Veces que rechazó</th><th className="w-[16%] px-1 py-1">Resultado</th><th className="w-[16%] px-1 py-1">Operador</th><th className="w-[26%] px-1 py-1">Causal / nota</th></tr></thead><tbody className="divide-y divide-slate-100">{callQueue.map((row) => <ClientRow key={row.id} row={row} draft={drafts[row.id] || {}} onOpen={() => setSelectedClient(row)} onOpenHistory={() => { const offender = offendersByCode.get(row.client_code.trim().toLowerCase()); if (offender) setSelectedOffender(offender); }} setDraft={(next) => setDrafts((current) => ({ ...current, [row.id]: { ...current[row.id], ...next } }))} save={(override) => void save(row, override)} saving={saving === row.id}/>)}</tbody></table></div>}
      </section>
      <section className="hidden">
        <div className="flex min-h-[64px] items-center justify-between border-b border-emerald-100 bg-gradient-to-r from-emerald-50/80 via-white to-white px-2.5 py-2"><div className="min-w-0"><p className="text-[7px] font-black uppercase tracking-[.14em] text-emerald-600">Tabla 3 · Volumen rechazado</p><h2 className="truncate text-sm font-black text-[#101c36]">Top 10 por cajas rechazadas</h2><p className="truncate text-[8px] text-slate-500">Cajas acumuladas y visitas con rechazo.</p></div><span className="ml-2 grid h-7 w-7 shrink-0 place-items-center rounded-md bg-emerald-100 text-[9px] font-black text-emerald-700">10</span></div>
        {loading ? <div className="grid min-h-0 flex-1 place-items-center text-[10px] font-semibold text-slate-500">Calculando rechazos...</div> : volumeClients.length === 0 ? <div className="grid min-h-0 flex-1 place-items-center p-6 text-center text-[10px] text-slate-500">No hay cajas rechazadas registradas.</div> : <div className="min-h-0 flex-1 overflow-auto"><table className="w-full min-w-[440px] table-fixed text-left text-[9px]"><thead className="sticky top-0 z-10 bg-slate-50 text-[7px] uppercase tracking-wide text-slate-500"><tr><th className="w-7 px-1.5 py-1.5">#</th><th className="w-[17%] px-1 py-1.5">Código</th><th className="w-[34%] px-1 py-1.5">Cliente</th><th className="w-[22%] px-1 py-1.5">Contratista</th><th className="w-[13%] px-1 py-1.5 text-right">Visitas</th><th className="w-[14%] px-1.5 py-1.5 text-right">Cajas</th></tr></thead><tbody className="divide-y divide-slate-100">{volumeClients.map((row, index) => <tr className="h-7 cursor-pointer hover:bg-emerald-50/60" key={`${row.contractor}-${row.client_code}`} onClick={() => setSelectedOffender(row)}><td className="px-1.5 py-0.5"><span className={`grid h-4 w-4 place-items-center rounded text-[7px] font-black ${index < 3 ? "bg-emerald-600 text-white" : "bg-slate-100 text-slate-500"}`}>{index + 1}</span></td><td className="truncate px-1 py-0.5 font-black text-[#101c36]">{row.client_code}</td><td className="truncate px-1 py-0.5 font-semibold text-slate-700" title={row.client_name}>{row.client_name || "Sin nombre"}</td><td className="truncate px-1 py-0.5 text-slate-500">{row.contractor}</td><td className="px-1 py-0.5 text-right"><span className="rounded-full bg-emerald-100 px-1 py-0.5 font-black text-emerald-700">{row.refusals}x</span></td><td className="px-1.5 py-0.5 text-right font-black text-rose-600">{row.rejected_boxes.toLocaleString("es-CO", { maximumFractionDigits: 2 })}</td></tr>)}</tbody></table></div>}
      </section>
      </div>
...
function ClientRow({ row, draft, onOpen, onOpenHistory, setDraft, save, saving }: { row: RecordRow; draft: { result?: "si" | "no"; reason?: string; notes?: string; editing?: boolean }; onOpen: () => void; onOpenHistory: () => void; setDraft: (value: Partial<typeof draft>) => void; save: (override?: { result?: "pendiente" | "si" | "no"; reason?: string; notes?: string }) => void; saving: boolean }) {
  ...
  const operatorName = row.last_edited_by || row.caller_name || "Sin asignar";
  const clientCell = <><button className="max-w-full text-left hover:text-blue-700" onClick={onOpen} title="Ver productos del pedido" type="button"><span className="block truncate font-bold text-[#101c36]">{row.client_name || "Sin nombre"}</span><span className="block truncate text-[8px] text-slate-500">{row.client_code} · {(row.products || []).length} productos · <strong className="text-blue-600">Ver pedido</strong></span></button></>;
  ...
  return <tr className={`h-[38px] ${draft.editing ? "bg-blue-50/60" : "hover:bg-slate-50/70"}`}><td className="truncate px-2 py-1">{draft.editing ? <span className="font-bold text-[#101c36]">{row.client_name || "Sin nombre"}</span> : clientCell}</td><td className="px-1 py-1">{rejectionButton}</td><td className="px-1 py-1">{resultButtons}</td><td className="truncate px-1 py-1 text-[9px] font-bold text-blue-700" title={operatorName}>{operatorName}</td><td className="px-1 py-1"><div className="flex gap-1">{contactReason}{draft.editing ? clearButton : null}{draft.editing ? cancelButton : null}</div></td></tr>;
}
