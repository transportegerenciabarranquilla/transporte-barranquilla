...
import { ArrowLeft, BarChart3, Check, Clock3, Pencil, Phone, RefreshCw, Search, Upload, Users, X } from "lucide-react";
...
type Offender = { contractor: string; client_code: string; client_name: string; phone: string; refusals: number; rejected_boxes: number };
...
export default function PreventaPage() {
  ...
  async function load() {
    setLoading(true);
    try {
      const [sessionResponse, dataResponse] = await Promise.all([fetch("/api/session/session", { cache: "no-store" }), fetch("/api/preventa", { cache: "no-store" })]);
  ...
  useEffect(() => { setCallerName(sessionStorage.getItem("preventa.caller-name") || ""); void load(); }, []);
  ...
  const modulationTableRows = useMemo(() => historicalOffenders.filter((row) => contractor === "Todas" || row.contractor === contractor).slice(0, 10), [historicalOffenders, contractor]);
  ...
  async function upload(file?: File) {
    ...
    try {
      ...
      setMessage(`${body.imported} clientes cargados desde ${body.fileName}.`); await load(); setBatch(body.batchId);
    ...
    <section className="mx-auto max-w-[1500px] space-y-5 px-4 py-6 sm:px-8">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div><p className="text-xs font-bold uppercase tracking-[.18em] text-violet-600">Base de llamadas</p><h2 className="mt-1 text-2xl font-bold text-[#101c36]">Clientes programados para preventa</h2><p className="mt-1 text-sm text-slate-500">Excel: codigo del cliente, nombre, telefono y cantidad de rechazos.</p></div>
      ...
      <section className="flex h-[620px] min-w-0 flex-col overflow-hidden rounded-xl border border-blue-100 bg-white shadow-[0_12px_30px_-24px_rgba(37,99,235,.4)]">
        <div className="flex min-h-[62px] items-center justify-between gap-2 border-b border-blue-100 bg-gradient-to-r from-blue-50/80 via-white to-white px-3 py-2"><div><p className="text-[8px] font-black uppercase tracking-[.16em] text-blue-600">Tabla 2 · Excel preventa</p><h2 className="text-sm font-black text-[#101c36]">Top 10 para gestionar</h2><p className="text-[9px] text-slate-500">La respuesta se guarda automáticamente.</p></div><div className="flex items-center gap-1.5"><span className="rounded-full bg-blue-100 px-2 py-1 text-[9px] font-black text-blue-700">{stats.pending} pendientes</span><button className="inline-flex h-7 items-center gap-1 rounded-md bg-blue-600 px-2 text-[9px] font-black text-white hover:bg-blue-700" disabled={loading || Boolean(saving)} onClick={() => void refreshCalls()} type="button"><RefreshCw size={11}/>Actualizar</button></div></div>
...
function ClientRow({ row, draft, onOpen, setDraft, save, saving }: { row: RecordRow; draft: { result?: "si" | "no"; reason?: string; notes?: string; editing?: boolean }; onOpen: () => void; setDraft: (value: Partial<typeof draft>) => void; save: (override?: { result?: "pendiente" | "si" | "no"; reason?: string; notes?: string }) => void; saving: boolean }) {
  ...
  return <tr className={`h-[38px] ${draft.editing ? "bg-blue-50/60" : "hover:bg-slate-50/70"}`}><td className="truncate px-2 py-1">{draft.editing ? <span className="font-bold text-[#101c36]">{row.client_name || "Sin nombre"}</span> : clientCell}</td><td className="truncate px-1 py-1 font-semibold">{row.phone || "Sin telefono"}</td><td className="px-1 py-1"><span className="rounded-full bg-violet-50 px-1.5 py-0.5 font-bold text-violet-700">{row.previous_refusals}</span></td><td className="px-1 py-1">{resultButtons}</td><td className="px-1 py-1"><div className="flex gap-1">{contactReason}{draft.editing ? clearButton : null}{draft.editing ? cancelButton : null}</div></td></tr>;
}
