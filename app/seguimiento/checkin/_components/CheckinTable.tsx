            ...
            {rows.length ? rows.map(({ checkin, key, resumen, vehicle }) => (
              <tr className="transition hover:bg-slate-50" key={`${vehicle.vehiculo}-${vehicle.transporte}`}>
                ...
                <td className="px-3 py-1.5">
                  ...
                  {savingDt === key ? <p className="mt-1 text-center text-[10px] font-semibold text-slate-500">Guardando…</p> : checkin && (inputs[key] === undefined || inputs[key] === String(checkin.totalCajas)) ? <p className="mt-1 text-center text-[10px] font-semibold text-emerald-700">Guardado</p> : savedDt === key ? <p className="mt-1 text-center text-[10px] font-semibold text-emerald-700">Guardado</p> : null}
                </td>
