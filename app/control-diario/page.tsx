...
function EffectiveRestTable({ rows }: { rows: EffectiveRestRow[] }) {
  ...
  const [timeFilter, setTimeFilter] = useState<"all" | "after-five">("all");
  const [roleFilter, setRoleFilter] = useState<"all" | "rr" | "driver" | "assistant">("all");
  const filteredRows = rows.filter((row) => {
    const detail = row.details[0];
    const matchesTime = timeFilter === "all" || detail.effectiveEntry > 5 * 3600;
    const role = attendanceRoleGroup(row.role);
    return matchesTime && (roleFilter === "all" || role === roleFilter);
  });
  const visibleRows = showAll ? filteredRows : filteredRows.slice(0, 10);
  return <article className="flex h-[500px] flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-lg lg:col-span-2 xl:col-span-1"><div className="shrink-0 border-b border-slate-100 bg-gradient-to-r from-emerald-50 to-white px-3 py-2"><div className="flex items-center justify-between"><div><p className="flex items-center gap-1 text-[8px] font-black uppercase tracking-[.12em] text-emerald-600"><Coffee size={11} />Entrada Descanso Efectivo</p><h3 className="text-sm font-black">Descanso efectivo</h3></div><span className="rounded-full bg-emerald-100 px-2 py-0.5 text-[10px] font-black text-emerald-700">{filteredRows.length}</span></div><div className="mt-2 grid grid-cols-2 gap-2"><select aria-label="Filtrar descanso por hora" className="h-7 rounded-lg border border-emerald-200 bg-white px-2 text-[9px] font-bold text-slate-700" onChange={(event) => { setTimeFilter(event.target.value as "all" | "after-five"); setShowAll(false); }} value={timeFilter}><option value="all">Todas las horas</option><option value="after-five">Después de 5:00 a. m.</option></select><select aria-label="Filtrar descanso por cargo" className="h-7 rounded-lg border border-emerald-200 bg-white px-2 text-[9px] font-bold text-slate-700" onChange={(event) => { setRoleFilter(event.target.value as "all" | "rr" | "driver" | "assistant"); setShowAll(false); }} value={roleFilter}><option value="all">Todos los cargos</option><option value="rr">RR</option><option value="driver">Conductores</option><option value="assistant">Auxiliares</option></select></div></div><div className={`min-h-0 flex-1 ${showAll ? "overflow-auto" : "overflow-hidden"}`}>{filteredRows.length ? visibleRows.map((row, index) => { const detail = row.details[0]; return <div className="grid h-[38px] grid-cols-[24px_1fr_auto] items-center gap-2 border-b border-slate-100 px-3" key={row.key}><span className={`grid h-6 w-6 place-items-center rounded-md text-[10px] font-black ${index < 3 ? "bg-emerald-500 text-white" : "bg-slate-100 text-slate-500"}`}>{index + 1}</span><span className="min-w-0 leading-none"><span className="block truncate text-[10px] font-black text-slate-800">{row.name}</span><span className="mt-0.5 block truncate text-[7px] font-bold uppercase text-slate-400">{row.role} · Salida {formatClockSeconds(detail.departure)}</span></span><span className="text-right"><strong className="block text-xs text-emerald-700">{formatClockSeconds(detail.effectiveEntry)}</strong><span className="block text-[7px] font-bold text-slate-400">Entrada {detail.effectiveEntryDate}</span></span></div>; }) : <div className="grid h-full place-items-center p-6 text-center text-sm text-slate-400">No hay descansos que coincidan con los filtros.</div>}</div>{filteredRows.length > 10 ? <button className="h-[42px] shrink-0 border-t border-slate-200 bg-white text-xs font-black text-emerald-700 hover:bg-emerald-50" onClick={() => setShowAll((value) => !value)} type="button">{showAll ? "Ver menos" : `Ver más (${filteredRows.length - 10})`}</button> : null}</article>;
}

function attendanceRoleGroup(value: unknown): "rr" | "driver" | "assistant" | "other" {
  const role = normalizeTemplateText(value);
  if (role.includes("conductor")) return "driver";
  if (role.includes("auxiliar")) return "assistant";
  if (role === "rr" || role.includes("responsable") || role.includes("lider de ruta") || role.includes("lider ruta")) return "rr";
  return "other";
}
