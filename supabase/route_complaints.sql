...
create policy "quejas lectura admin logisticos" on public.route_complaints for select to authenticated using (
  ...
  or (lower(coalesce(auth.jwt() ->> 'email', '')) = 'logisticos@bavaria-seguimiento.com' and contractor in ('Logisticos', 'Punto Corona', 'Surti Cervezas'))
...
set
  ...
  data = jsonb_set(
    jsonb_set(q.data, '{contractor}', to_jsonb(public.resolve_complaint_contractor(q.dt, q.created_date, q.contractor)), true),
    '{matched}',
    'true'::jsonb,
    true
  )
